 ...
 * @author bluax
 */
@WebServlet(name = "updateCarServlet", urlPatterns = { Routes.UPDATE_CAR })
public class updateCarServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            CarDAO d = new CarDAO();
            Car car = d.updateCar(id, serial, model, colour, year);
            
            if(car == null){
                request.setAttribute("updateStatus", "Car ID not found");
            }else{
                request.setAttribute("updateStatus", "Update successfull");
            }

            request.getRequestDispatcher(Pages.UPDATE_CAR_PAGE).forward(request, response);
        }
    }
